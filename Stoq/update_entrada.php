...
function getLocalizacoesDisponiveisComDetalhes($conn, $produto_id) {
    ...
    if ($produto) {
        ...
        foreach ($localizacoes as $localizacao) {
            ...
            if ($produtoVolumecubo <= $localizacaoVolume) { 
                $localizacaovolume[] = $localizacao['volume_armazem'];
                }           
            }
            
            if(!empty($localizacaovolume)){
    //Array de localização       
    $meuArray = $localizacaovolume;
    $menorValor = sort($meuArray);
    $locs = $meuArray[0];
            }
            if(!empty($locs)){
    //Array de Produto
    foreach ($localizacoes as $localizacao) {
        if(($altp <= $alt) and ($largp <= $larg) and ($compp <= $comp )){
            ...
            <div class="item">
                ...
                <table class="table">
                    ...
                    <?php    
                          $localizacoesDisponiveis = getLocalizacoesDisponiveisComDetalhes($conn, $produto_id);
                          if (!empty($localizacoesDisponiveis)) {
                            $selec = $conn->prepare("SELECT id_armazem, nm_armazem, rua_armazem, baia_armazem, nv_armazem, vao_armazem, volume_armazem FROM tbdcadloc WHERE id_armazem = :alocar");
